 ...
 */
export function buildStoragePath(organizationId, folder, filename) {
 ...
 */
export function extractOrganizationIdFromPath(path) {
 ...
 */
export function pathBelongsToOrganization(path, organizationId) {
 ...
 */
export function buildPublicUrl(bucketUrl, path) {
 ...
 */
export function extractPathFromUrl(url, bucketUrl) {
    const cleanBucketUrl = bucketUrl.endsWith('/')
        ? bucketUrl.slice(0, -1)
        : bucketUrl;
 ...
 */
export function buildListPrefix(organizationId, folder) {
    return folder
