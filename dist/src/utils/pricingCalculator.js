...
function roundPrice(value, rounding = "nearest") {
    switch (rounding) {
        case "up":
...
function applyTax(amountHT, taxRate) {
 ...
 */
function calculatePerDay(context) {
    ...
    return {
        ...
        discount_amount: discountAmount > 0 ? roundPrice(discountAmount) : undefined,
 ...
 */
function calculateFlatRate(context) {
    const config = context.pricing_rule?.calculation_config || {};
    const taxRate = config.tax_rate ?? context.business_rules?.pricing?.tax_rate ?? 20;
    const rounding = config.rounding || "nearest";
 ...
 */
function calculateFixedPrice(context) {
    const config = context.pricing_rule?.calculation_config || {};
    const taxRate = config.tax_rate ?? context.business_rules?.pricing?.tax_rate ?? 20;
 ...
 */
function calculateTiered(context) {
    const config = context.pricing_rule?.calculation_config || {};
    const taxRate = config.tax_rate ?? context.business_rules?.pricing?.tax_rate ?? 20;
    const rounding = config.rounding || "nearest";
 ...
 */
export function calculatePrice(context) {
    // Calculate duration if not provided
    if (!context.rental.duration_days) {
        context.rental.duration_days = calculateDays(context.rental.start_date, context.rental.end_date);
    }
    // Determine strategy to use
    const strategy = context.pricing_rule?.strategy ||
 ...
 */
export function isPricingRuleApplicable(rule, context) {
    if (!rule.is_active)
        return false;
    if (!rule.applies_to)
