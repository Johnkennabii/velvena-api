 ...
 */
export async function moveMailFolder(fromName, toName) {
    return new Promise((resolve, reject) => {
        const imap = createImapConnection();
        let timeoutId = null;
        const clear = () => {
 ...
 */
export async function replyToEmail(uid, mailboxType, replyBody, replyBodyText, attachments) {
...
<div style="border-left: 2px solid #ccc; padding-left: 10px; margin-top: 20px; color: #666;">
  ...
  ${originalEmail.html || (originalEmail.text ? `<p>${originalEmail.text.replace(/\n/g, '<br/>')}</p>` : "")}
    ...
    const quotedText = replyBodyText
        ...
        : undefined;
    await sendEmail(replyTo, replySubject, quotedHtml, quotedText, undefined, undefined, attachments);
    await addFlag(uid, "\\Answered", mailboxType);
}
/**
 * Répondre à tous (expéditeur + tous les destinataires originaux)
 */
export async function replyAllToEmail(uid, mailboxType, replyBody, replyBodyText, attachments) {
    const originalEmail = await getEmailByUid(uid, mailboxType);
    if (!originalEmail) {
        throw new Error("Email original introuvable");
    }
...
<div style="border-left: 2px solid #ccc; padding-left: 10px; margin-top: 20px; color: #666;">
  ...
  ${originalEmail.html || (originalEmail.text ? `<p>${originalEmail.text.replace(/\n/g, '<br/>')}</p>` : "")}
    ...
    const quotedText = replyBodyText
        ? `${replyBodyText}\n\n--- Le ${originalDate}, ${originalFrom} a écrit ---\n${originalTextContent}`
        : undefined;
    await sendEmail(replyTo, replySubject, quotedHtml, quotedText, filteredCc.length > 0 ? filteredCc : undefined, undefined, attachments);
    await addFlag(uid, "\\Answered", mailboxType);
}
/**
 * Transférer un email
 */
export async function forwardEmail(uid, mailboxType, to, forwardMessage, forwardMessageText, includeAttachments = true) {
    const originalEmail = await getEmailByUid(uid, mailboxType);
    if (!originalEmail) {
        throw new Error("Email original introuvable");
    }
...
<div>
  ${originalEmail.html || (originalEmail.text ? `<p>${originalEmail.text.replace(/\n/g, '<br/>')}</p>` : "")}
    ...
    await sendEmail(to, forwardSubject, forwardHeader, forwardTextHeader, undefined, undefined, attachments);
}
