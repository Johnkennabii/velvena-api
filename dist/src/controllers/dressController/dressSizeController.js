...
// GET all dress sizes
export const getDressSizes = async (_req, res) => {
    try {
...
// CREATE a new size
export const createDressSize = async (req, res) => {
    try {
        const { name } = req.body;
        if (!name)
...
// SOFT DELETE
export const softDeleteDressSize = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ success: false, error: "ID is required" });
...
// HARD DELETE
export const hardDeleteDressSize = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id)
            return res.status(400).json({ success: false, error: "ID is required" });
...
//# sourceMappingURL=dressSizeController.js.map