...
// GET all colors
export const getDressColors = async (_req, res) => {
    try {
...
// CREATE
export const createDressColor = async (req, res) => {
    try {
...
// UPDATE
export const updateDressColor = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
...
// SOFT DELETE
export const softDeleteDressColor = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, error: "Color id is required" });
...
// HARD DELETE
export const hardDeleteDressColor = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, error: "Color id is required" });
        }
        await prisma.dressColor.delete({ where: { id: id } });
...
//# sourceMappingURL=dressColorController.js.map