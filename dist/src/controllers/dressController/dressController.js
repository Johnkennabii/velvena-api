...
export const createDress = async (req, res) => {
    try {
        ...
        const creatorFullName = creatorFirstName || creatorLastName
            ? [creatorFirstName, creatorLastName].filter((value) => Boolean(value)).join(" ")
            : null;
...
export const updateDress = async (req, res) => {
    try {
        ...
        const { name, reference, price_ht, price_ttc, price_per_day_ht, price_per_day_ttc, type_id, size_id, condition_id, color_id, published_post, } = body;
        ...
        const files = req.files || [];
        pino.info({ files: files.map(f => ({ originalname: f.originalname, mimetype: f.mimetype, size: f.size })) }, "ℹ️ Uploaded files info");
...
// PUBLISH DRESS
export const publishDress = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, error: "ID is required" });
...
// UNPUBLISH DRESS
export const unpublishDress = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({ success: false, error: "ID is required" });
...
export const addDressImages = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id)
...
export const removeDressImage = async (req, res) => {
    try {
        const { id } = req.params;
        const collectKeys = () => {
            const raw = [];
            const addValue = (value, splitComma = false) => {
            ...
            addValue(req.params?.key);
            addValue(req.params?.keys, true);
            return Array.from(new Set(raw));
        };
...
export const getDressesAvailability = async (req, res) => {
    try {
      ...
      `);
        ...
        pino.error({ message: error.message, stack: error.stack }, "❌ Failed to fetch dress availability");
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=dressController.js.map