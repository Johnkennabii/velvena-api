 ...
 */
export const addDressReservations = async (req, res) => {
    try {
        ...
        const reservations = await Promise.all(dresses.map(async (dress) => {
        ...
        pino.info({ prospectId, count: reservations.length }, "✅ Dress reservations added to prospect");
        res.status(201).json({
            success: true,
            data: reservations,
        });
 ...
 */
export const getProspectReservations = async (req, res) => {
    try {
        const { prospectId } = req.params;
        if (!prospectId) {
            return res.status(400).json({
...
//# sourceMappingURL=prospectDressReservationController.js.map