 ...
 */
function calculateDressEstimates(dresses) {
    return dresses.map((dressData) => {
 ...
 */
export const createProspectRequest = async (req, res) => {
    try {
        ...
        const dressIds = dresses.map((d) => d.dress_id);
        ...
        const dressesWithCalculations = dresses.map((dressData) => {
        ...
        pino.info({ prospectId, requestId: request.id, requestNumber }, "✅ Prospect request created");
        res.status(201).json({
            success: true,
            data: request,
        });
 ...
 */
export const getProspectRequests = async (req, res) => {
    try {
        const { prospectId } = req.params;
        if (!prospectId) {
            return res.status(400).json({
 ...
 */
export const getProspectRequestById = async (req, res) => {
    try {
 ...
 */
export const updateProspectRequest = async (req, res) => {
    try {
        const { prospectId, requestId } = req.params;
 ...
 */
export const deleteProspectRequest = async (req, res) => {
    try {
        const { prospectId, requestId } = req.params;
        if (!prospectId || !requestId) {
            return res.status(400).json({
...
//# sourceMappingURL=prospectRequestController.js.map