...
import { calculatePrice, findBestPricingRule } from "../utils/pricingCalculator.js";
/**
 * GET /pricing-rules
...
export const getPricingRules = async (req, res) => {
    try {
        ...
        const where = withOrgOrGlobal(req.user.organizationId, {
            deleted_at: null,
            is_active: true,
        });
        if (service_type_id) {
            where.AND.push({ service_type_id: service_type_id });
        }
        if (strategy) {
            where.AND.push({ strategy: strategy });
        }
...
export const createPricingRule = async (req, res) => {
    try {
        ...
        const { name, service_type_id, strategy, calculation_config, applies_to, priority = 0, } = req.body;
        if (!name || !strategy) {
        ...
        logger.info({ pricingRuleId: pricingRule.id, organizationId: req.user.organizationId }, "Pricing rule created");
        res.status(201).json({
            success: true,
            data: pricingRule,
        });
...
export const updatePricingRule = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const { name, service_type_id, calculation_config, applies_to, priority, is_active, } = req.body;
        // Check ownership
        ...
        logger.info({ pricingRuleId: pricingRule.id, organizationId: req.user.organizationId }, "Pricing rule updated");
        res.json({
            success: true,
            data: pricingRule,
        });
...
export const deletePricingRule = async (req, res) => {
    try {
        ...
        logger.info({ pricingRuleId: id, organizationId: req.user.organizationId }, "Pricing rule deleted");
        res.json({
            success: true,
...
export const calculatePriceEndpoint = async (req, res) => {
    try {
        ...
        const { dress_id, start_date, end_date, pricing_rule_id, overrides, } = req.body;
        ...
        if (pricing_rule_id) {
            ...
            pricingRule = findBestPricingRule(allRules, {
