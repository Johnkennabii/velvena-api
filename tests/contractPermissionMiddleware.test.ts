...
afterEach(() => {
  (prisma.contract.findUnique as any) = originalFindUnique;
});
...
test("middleware bloque un MANAGER sur un contrat signé", async () => {
  (prisma.contract.findUnique as any) = async () => ({
    id: "contract-1",
    status: "SIGNED",
  ...
  const req: any = { params: { id: "contract-1" }, user: { id: "user-1", role: "MANAGER" } };
  const res: any = {
    statusCode: 200,
    payload: null as any,
    status(code: number) {
...
test("middleware bloque un COLLABORATOR sur un contrat non brouillon", async () => {
  ...
  const middleware = contractPermissionMiddleware();
  const req: any = { params: { id: "contract-2" }, user: { id: "user-2", role: "COLLABORATOR" } };
  const res: any = {
    statusCode: 200,
    payload: null as any,
    status(code: number) {
...
test("middleware laisse passer un ADMIN quel que soit le statut", async () => {
  ...
  const middleware = contractPermissionMiddleware();
  const req: any = { params: { id: "contract-3" }, user: { id: "user-3", role: "ADMIN" } };
  const res: any = {
    statusCode: 200,
    payload: null as any,
    status(code: number) {
